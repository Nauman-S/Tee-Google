...
func TestLinuxConnBuffers(t *testing.T) {
	...
	c, err := socket3.Socket(unix.AF_INET, unix.SOCK_STREAM, 0, "tcpv4", nil)
	if err != nil {
...
func TestLinuxDialVsockNoListener(t *testing.T) {
	...
	c, err := socket3.Socket(unix.AF_VSOCK, unix.SOCK_STREAM, 0, "vsock", nil)
	if err != nil {
		t.Fatalf("failed to open socket: %v", err)
	}
	defer c.Close()
